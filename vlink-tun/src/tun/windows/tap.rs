...
// use crate::{packet, Tun};
use crate::tun::Error;
use crate::tun::windows::{ctl_code, decode_utf16, encode_utf16, ffi, netsh};
...
impl Device {
    ...
    pub fn new(name: String) -> io::Result<Self> {
        ...
        let name = ffi::luid_to_alias(&luid).map(|name| decode_utf16(&name))?;
        let device = Self {
            handle,
            index,
            luid,
...
impl Tun for Device {
    ...
    fn mtu(&self) -> Result<u16, Error> {
        let mut mtu = 0;
        Ok(ffi::device_io_control(self.handle, TAP_WIN_IOCTL_GET_MTU, &(), &mut mtu).map(|_| mtu)?)

    }

    fn set_mtu(&self, mtu: u16) -> Result<(), Error> {
        Ok(netsh::set_interface_mtu(self.index, mtu as u32)?)
    }
    ...
    fn set_address(&self, value: Ipv4Addr) -> io::Result<()> {
        ...
        netsh::set_interface_ip(self.index, &value, &a)
    }
