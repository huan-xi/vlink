...
use crate::device::cipher::chacha20_poly1305_cipher::ChachaCipher;
...
pub struct Session {
    ...
    created_at: Instant,
    encrypt_cipher: ChachaCipher,
    decrypt_cipher: ChachaCipher,
}
...
impl Session {
    ...
    pub fn new(
        ...
        let encrypt_cipher = ChachaCipher::new(&sender_key).unwrap();
        let decrypt_cipher = ChachaCipher::new(&receiver_key).unwrap();
        Self {
            secret,
            sender_index,
            sender_key,
    ...
    pub fn decrypt_data(&self, packet: &protocol::TransportData) -> Result<Vec<u8>, Error> {
        ...
        self.decrypt_cipher.aead_decrypt(packet.counter, &packet.payload, &[]).map_err(Error::Noise)
    }
